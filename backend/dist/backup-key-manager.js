...
import { BACKUP_KEY_ALGORITHM, BACKUP_KEY_KDF, getBackupEncryptionState, getCurrentBackupKey, getCandidateBackupKeys, rotateBackupEncryptionKey, currentKeyFilePath, keyRingDir, BackupKeyError, } from "./services/backupKeyManager.js";
import { generateBackupKey, deriveKeyId, probeBackupFile, encryptBackupFile, decryptBackupFile, BackupCryptoError, } from "./services/backupCrypto.js";
    ...
    switch (command) {
        case "generate": {
            ...
            if (flags.output) {
                ...
                fs.writeFileSync(flags.output, `# ZKVOTE backup encryption key (v1)\n${key}\n`, { encoding: "utf-8", mode: 0o600 });
                fs.chmodSync(flags.output, 0o600);
                console.info(`Wrote key to ${flags.output} (mode 600)`);
            }
            else {
                console.warn("WARNING: key printed to stdout. Use --output or BACKUP_ENCRYPTION_KEY in production.");
            }
            return;
        }
        case "status":
        ...
        case "import-key": {
            const rawKey = rest[0];
            if (!rawKey) {
                console.error("Usage: tsx src/backup-key-manager.ts import-key <base64key>");
                process.exit(1);
            }
            ...
            if (!fs.existsSync(archivePath)) {
                fs.writeFileSync(archivePath, `# ZKVOTE archived backup encryption key (${keyId})\n${rawKey}\n`, { encoding: "utf-8", mode: 0o600 });
        ...
        case "encrypt": {
            ...
            if (!key) {
                console.error("ERROR: no key configured. Run 'generate' or pass --key <key>.");
                process.exit(1);
            }
        ...
        case "decrypt": {
            const input = requiredFlag(flags, "input");
            const output = flags.output || input.replace(/\.enc\.db$/, ".db") || `${input}.db`;
            ...
            console.error(`ERROR: unable to decrypt: ${lastError.message}`);
            process.exit(1);
            return;
        }
        case "verify": {
            ...
            if (result.valid) {
                ...
                console.info(`Key id: ${result.keyId ?? info.keyId} | integrity: ${result.integrityResult}`);
            }
            else {
        ...
        case "restore-test": {
            const input = requiredFlag(flags, "input");
            console.info(`Running encrypted restore drill on ${path.basename(input)}...`);
...
//# sourceMappingURL=backup-key-manager.js.map