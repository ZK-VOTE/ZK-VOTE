...
const envBool = (defaultVal = false) => z
    .string()
    .optional()
    .transform((val) => {
    if (val === undefined)
...
const stellarContractId = z
    .string()
    .regex(/^C[A-Z2-7]{55}$/, "Must be a 56-character Stellar contract ID (C + 55 base32 chars)");
// ============================================
...
export const configSchema = z
    ...
    SOROBAN_RPC_URLS: commaSeparatedArray
        .describe("Comma-separated list of RPC URLs for load balancing. Falls back to SOROBAN_RPC_URL"),
    ...
    // ── Contract IDs ────────────────────────────────────────────
    VOTING_CONTRACT_ID: stellarContractId.describe("Stellar contract ID for the Voting contract"),
    TREE_CONTRACT_ID: stellarContractId.describe("Stellar contract ID for the MembershipTree contract"),
    COMMENTS_CONTRACT_ID: stellarContractId.describe("Stellar contract ID for the Comments contract"),
    DAO_REGISTRY_CONTRACT_ID: stellarContractId.optional().describe("Stellar contract ID for the DaoRegistry contract"),
    MEMBERSHIP_SBT_CONTRACT_ID: stellarContractId.optional().describe("Stellar contract ID for the MembershipSBT contract"),
    BRIDGE_CONTRACT_ID: stellarContractId.optional().describe("Stellar contract ID for the Bridge contract"),
    CIRCUIT_REGISTRY_CONTRACT_ID: stellarContractId.optional().describe("Stellar contract ID for the CircuitRegistry contract"),
    // ── VK Version ──────────────────────────────────────────────
    ...
    PINATA_GATEWAY: optionalString.describe("Custom Pinata gateway URL (default: gateway.pinata.cloud)"),
    // ── IPFS Pin Redundancy ─────────────────────────────────────
    IPFS_BACKUP_DIR: z
        .string()
        .default("./data/ipfs-backup")
        .describe("Local directory for content backups before pinning"),
    WEB3_STORAGE_TOKEN: optionalString.describe("Web3.Storage API token for secondary pinning"),
 ...
 */
export function detectConfigChanges(previous, current) {
    const changes = [];
 ...
 */
export function isHotReloadable(key) {
 ...
 */
export function applyHotReload(current, updates) {
