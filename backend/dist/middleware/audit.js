...
export function createCorsOptions(rawOrigins) {
...
function inferAction(path, method) {
    ...
    if (path.includes("/tally") || path.includes("verify_tally_proof"))
        return "tally_verify";
...
export function exportAuditLogs(format = "json") {
    if (format === "csv") {
        ...
        const rows = auditEntries.map((e) => [e.id, e.timestamp, e.requestId, e.method, e.path, e.action, e.actor, e.statusCode ?? "", e.durationMs ?? ""]
...
export function auditMiddleware(req, res, next) {
    ...
    res.on("finish", () => {
        try {
            // Read the body at finish time: JSON bodies are parsed by per-route
            // `bodyLimit` middleware *after* auditMiddleware runs, so req.body is
            // only populated here, once the request has been handled.
...
export function auditAction(req, action, extra) {
    ...
    return appendAudit({
        ...
        actor,
        actorIpHash: hashIp(req.ip),
        requestBody: extra?.body !== undefined
