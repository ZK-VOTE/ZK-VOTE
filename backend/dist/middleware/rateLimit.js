...
export function getRateLimitMetrics() {
    ...
    for (const [name, m] of Object.entries(rateLimitMetrics)) {
        out[name] = {
            ...m,
            blockRate: m.total > 0 ? Math.round((m.blocked / m.total) * 100) / 100 : 0,
        };
    }
    return out;
}
 ...
 */
function withMetrics(name, limiter) {
    return (req, res, next) => {
        recordRequest(name);
...
export const walletRateLimiter = isTestMode
    ...
    : rateLimit({
        ...
        max: 5,
        message: {
            error: "Too many proof submissions for this wallet address, please try again later",
        },
        standardHeaders: true,
        legacyHeaders: true,
...
export const voteLimiter = isTestMode
    ? noopMiddleware
    : withMetrics("vote", rateLimit({
        windowMs: 60 * 1000, // 1 minute
        max: 10,
        ...headerOptions,
        store: getStore("vote"),
        keyGenerator: walletKeyGenerator,
        handler: makeHandler("vote", "Too many vote requests, please try again later"),
    }));
/**
...
export const queryLimiter = isTestMode
    ? noopMiddleware
    : withMetrics("query", rateLimit({
        windowMs: 60 * 1000, // 1 minute
        max: 60,
        ...headerOptions,
        store: getStore("query"),
        keyGenerator,
        handler: makeHandler("query", "Too many requests, please try again later"),
    }));
/**
...
export const ipfsUploadLimiter = isTestMode
    ? noopMiddleware
    : withMetrics("ipfsUpload", rateLimit({
        windowMs: 60 * 1000, // 1 minute
        max: 10,
        ...headerOptions,
        store: getStore("ipfsUpload"),
        keyGenerator,
        handler: makeHandler("ipfsUpload", "Too many upload requests, please try again later"),
    }));
/**
...
export const ipfsReadLimiter = isTestMode
    ? noopMiddleware
    : withMetrics("ipfsRead", rateLimit({
        windowMs: 60 * 1000, // 1 minute
        max: 200,
        ...headerOptions,
        store: getStore("ipfsRead"),
        keyGenerator,
        handler: makeHandler("ipfsRead", "Too many requests, please try again later"),
    }));
/**
...
export const commentLimiter = isTestMode
    ? noopMiddleware
    : withMetrics("comment", rateLimit({
        windowMs: 60 * 1000, // 1 minute
        max: 20,
        ...headerOptions,
        store: getStore("comment"),
        keyGenerator,
        handler: makeHandler("comment", "Too many comment requests, please try again later"),
    }));
/**
...
export const graduatedSlowDown = isTestMode
    ...
    : slowDown({
        windowMs: 60 * 1000,
        delayAfter: 40,
        delayMs: (used) => Math.min((used - 40) * 100, 3000),
        maxDelayMs: 3000,
 ...
 */
const memberKeyGenerator = (req) => {
    const member = req.body?.caller ||
 ...
 */
export function createPerMemberLimiter(opts) {
    const structuredHandler = makeHandler(opts.name, opts.message);
    return withMetrics(opts.name, rateLimit({
...
export const commitmentRegistrationLimiter = isTestMode
    ...
    : createPerMemberLimiter({
        ...
        message: "Too many commitment registrations for this member, please try again later",
...
export const verifyTallyProofLimiter = isTestMode
    ? noopMiddleware
    : withMetrics("verifyTallyProof", rateLimit({
        windowMs: 60 * 1000, // 1 minute
        max: 30,
        ...headerOptions,
