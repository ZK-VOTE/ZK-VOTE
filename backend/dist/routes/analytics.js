...
async function simulateTreeCall(method, daoId) {
    const contract = new StellarSdk.Contract(config.treeContractId);
    const operation = contract.call(method, StellarSdk.nativeToScVal(daoId, { type: "u64" }));
...
router.get("/analytics/:daoId", async (req, res) => {
