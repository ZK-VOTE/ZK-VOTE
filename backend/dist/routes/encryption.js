...
import { z } from "zod";
import { CONTENT_TYPES, ENVELOPE_VERSION, getActiveEpoch, getRecoveryShares, getWrappedKey, loadCiphertext, nextEpoch, recordGroupKeyEpoch, redactContent, redactEnvelopeForLog, storeCiphertext, } from "../services/encryption.js";
import { authGuard, commentLimiter, queryLimiter, validateBody, validateParams, } from "../middleware/index.js";
 ...
 * narrow the key.
 */
router.get(`${BASE}/daos/:daoId/epoch`, queryLimiter, validateParams(daoParams), (async (req, res) => {
    const { daoId } = params(req);
 ...
 * a key they hold.
 */
router.post(`${BASE}/daos/:daoId/epoch`, commentLimiter, authGuard, validateParams(daoParams), validateBody(rotationSchema), (async (req, res) => {
    const { daoId } = params(req);
    const body = req.body;
 ...
 */
router.get(`${BASE}/daos/:daoId/members/:memberId/key`, queryLimiter, authGuard, validateParams(memberParams), (async (req, res) => {
    const { daoId, memberId } = params(req);
    const epoch = getActiveEpoch(daoId);
    if (!epoch) {
        return res.status(404).json({ error: "No key epoch for this DAO" });
    }
...
router.get(`${BASE}/daos/:daoId/recovery-shares`, queryLimiter, authGuard, validateParams(daoParams), (async (req, res) => {
    const { daoId } = params(req);
    const epoch = getActiveEpoch(daoId);
    if (!epoch) {
        return res.status(404).json({ error: "No key epoch for this DAO" });
 ...
 */
router.get(`${BASE}/daos/:daoId/content/:contentType/:contentId`, queryLimiter, validateParams(contentParams), (async (req, res) => {
    const { daoId, contentType, contentId } = params(req);
 ...
 * cannot read.
 */
router.delete(`${BASE}/daos/:daoId/content/:contentType/:contentId`, commentLimiter, authGuard, validateParams(contentParams), validateBody(redactionSchema), (async (req, res) => {
    const { daoId, contentType, contentId } = params(req);
