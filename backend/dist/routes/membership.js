...
import { server, relayerKeypair, callWithTimeout, simulateWithBackoff, u256ToScVal, } from "../services/stellar.js";
import { authGuard, commitmentRegistrationLimiter, validateBody, } from "../middleware/index.js";
import { membershipRegisterSchema } from "../validation/schemas.js";
import { membershipRegistrationTotal, membershipRegistrationLimited, } from "../services/metrics.js";
const router = Router();
/**
 * Map a membership-tree contract error to an HTTP response.
 * Returns null when the call actually succeeded (no contract error).
 */
function classifyTreeError(errorStr) {
    const includes = (...cands) => cands.some((c) => errorStr.includes(String(c)));
    // #371: on-chain cooldown exceeded → 429 with retry guidance
    if (includes("RateLimited") || includes("#17") || includes("17")) {
        return {
            status: 429,
            message: "Rate limited: too many registrations for this member, try again after the cooldown window",
        };
    }
 ...
 */
router.post("/membership/register-commitment", authGuard, commitmentRegistrationLimiter, validateBody(membershipRegisterSchema), (async (req, res) => {
    ...
    try {
        ...
        const simResult = await callWithTimeout(() => simulateWithBackoff(() => server.simulateTransaction(tx)), "simulate_membership_register");
        if (!StellarSdk.rpc.Api.isSimulationSuccess(simResult)) {
            ...
            if (errorStr.toLowerCase().includes("auth") ||
                errorStr.includes("require_auth")) {
                return res.status(400).json({
                    error: "Registration must be authorized by the caller address; complete signing in your wallet",
                    prepared: false,
                });
            }
            ...
            if (classified) {
                ...
                membershipRegistrationLimited.inc({
                    reason: classified.status === 429 ? "contract_cooldown" : "rejected",
                });
                return res.status(classified.status).json({
                    error: classified.message,
                    limiter: classified.status === 429 ? "commitmentRegistration" : undefined,
                });
            }
        ...
        const authEntry = simResult.result?.auth?.map((entry) => entry.toXDR("base64"));
        res.json({
            success: true,
            prepared: true,
            daoId,
