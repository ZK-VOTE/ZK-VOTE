 ...
 */
router.post("/aggregate", bodyLimit("100kb"), validateBody(novaAggregateSchema), async (req, res) => {
    try {
 ...
 */
router.post("/verify", bodyLimit("100kb"), async (req, res) => {
    try {
        ...
        if (!payload ||
            ...
            return res.status(400).json({
                error: "Invalid payload. proof_bytes, initial_state, and final_state are required.",
            });
        }
