...
import { authGuard, auditLog, bodyLimit, queryLimiter, validateParams, validateBody, } from "../middleware/index.js";
import { computeVdf, verifyVdf, deriveVdfInput, DEFAULT_VDF_ITERATIONS, MIN_VDF_ITERATIONS, MAX_VDF_ITERATIONS, } from "../services/vdf.js";
import { daoParamsSchema } from "../validation/schemas.js";
import { z } from "zod";
const router = Router();
const orderingStore = new Map();
// ─── Zod schemas ─────────────────────────────────────────────────────────────
 ...
 */
router.post("/randomness/seed", bodyLimit("32kb"), authGuard, auditLog("randomness_seed"), validateBody(seedBodySchema), (async (req, res) => {
    const { daoId, proposalIds, blockHashHex, adminSeedHex, iterations, requiredShares, } = req.validatedBody;
    try {
        ...
        const blockHash = blockHashHex ?? crypto.randomBytes(32).toString("hex");
        const adminSeed = adminSeedHex ?? crypto.randomBytes(32).toString("hex");
        // First proposal ID is used as a pseudo-proposal-ID for vdf input derivation
        const vdfInput = deriveVdfInput(daoId, normalizedIds[0] ?? 0, blockHash, adminSeed);
        log("info", "randomness_vdf_start", { daoId, iterations });
        const { output: vdfOutput, checkpoints } = computeVdf(vdfInput, iterations);
 ...
 * Auth: required
 */
router.post("/randomness/contribute", bodyLimit("8kb"), authGuard, auditLog("randomness_contribute"), validateBody(contributeBodySchema), (async (req, res) => {
    const { daoId, authorityId, shareHex } = req
        .validatedBody;
    try {
        const state = orderingStore.get(daoId);
        if (!state) {
            return res.status(404).json({
                error: "No pending randomness computation for this DAO. Call /randomness/seed first.",
            });
        }
        if (state.committed) {
            return res.status(409).json({
        ...
        res.json({
            ...
            status: state.shares.length >= state.requiredShares
                ? "ready_to_finalize"
                : "awaiting_shares",
        });
    }
 ...
 * Auth: required
 */
router.post("/randomness/finalize", bodyLimit("4kb"), authGuard, auditLog("randomness_finalize"), validateBody(finalizeBodySchema), (async (req, res) => {
    const { daoId } = req.validatedBody;
    try {
        const state = orderingStore.get(daoId);
        if (!state) {
            return res.status(404).json({
        ...
        const authorityContribution = combineShares(state.shares.map((s) => s.shareHex));
 ...
 */
router.get("/randomness/ordering/:daoId", queryLimiter, validateParams(daoParamsSchema), (async (req, res) => {
    const { daoId } = req.validatedParams;
    try {
        const state = orderingStore.get(daoId);
        if (!state || state.finalizedAt === null) {
            return res.status(404).json({
                error: "No finalized ordering found for this DAO. Ordering may not have been computed yet.",
            });
        }
        res.json({
            daoId,
 ...
 * Auth: public (rate-limited)
 */
router.get("/randomness/verify/:daoId", queryLimiter, validateParams(daoParamsSchema), (async (req, res) => {
    const { daoId } = req.validatedParams;
    try {
        const state = orderingStore.get(daoId);
        if (!state || state.finalizedAt === null) {
        ...
        const vdfValid = verifyVdf(state.vdfInput, state.vdfIterations, state.vdfOutput, state.vdfCheckpoints);
        // 2. Verify replay nonce matches
        const recomputedNonce = computeReplayNonce(daoId, state.proposalIds, state.vdfOutput);
        const nonceValid = recomputedNonce === state.replayNonce;
        // 3. Re-derive the ordering from the combined seed and verify it matches
        const recomputedOrdering = seededShuffle(state.proposalIds, state.combinedSeed);
        const orderingValid = JSON.stringify(recomputedOrdering) === JSON.stringify(state.ordering);
