...
import { appendAudit, isIdempotencyKeyUsed, markIdempotencyKey, deriveActor, redactPii, } from "../middleware/audit.js";
...
export function getRemediationById(id) {
    return remediationLog.find((r) => r.id === id);
}
// ============================================
// VALIDATION
// ============================================
function validateRemediationBody(body) {
 ...
 */
router.post("/remediation/action", authGuard, (async (req, res) => {
    ...
    if (isIdempotencyKeyUsed(idempotencyKey)) {
        ...
        appendAudit({
            requestId: req.ctx || "unknown",
            method: req.method,
            path: req.path,
            action: "remediation_replay_blocked",
            actor: deriveActor(req),
            actorIpHash: hashIp(req.ip),
            requestBody: redactPii({ action, target, reason: reason.slice(0, 50), idempotencyKey: "[REDACTED]" }),
            statusCode: 409,
        });
    ...
    appendAudit({
        ...
        actor,
        actorIpHash: ipHash,
        requestBody: redactPii({ action, target, reason, idempotencyKey: "[REDACTED]", metadata }),
        statusCode: 201,
    });
 ...
 */
router.get("/remediation/log", authGuard, queryLimiter, (req, res) => {
    const { action, target, from, to, limit, offset } = req.query;
    let filtered = [...remediationLog];
    if (action)
        filtered = filtered.filter((r) => r.action === String(action));
    if (target)
        filtered = filtered.filter((r) => r.target === String(target));
    if (from) {
 ...
 */
router.get("/remediation/:id", authGuard, queryLimiter, (req, res) => {
    const { id } = req.params;
 ...
 */
router.post("/remediation/verify", authGuard, (req, res) => {
