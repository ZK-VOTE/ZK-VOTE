...
import { queryLimiter, validateBody, bodyLimit, } from "../middleware/index.js";
import { computeWeight, ageDaysAt, analyzeSybilBound, weightCurveTable, weightCurveParams, MAX_REPUTATION, } from "../services/sybil.js";
const router = Router();
// --- Validation schemas ---
 ...
 */
router.get("/sybil/params", queryLimiter, (async (_req, res) => {
    return res.json({
        ...weightCurveParams(),
 ...
 * snapshot.
 */
router.post("/sybil/weight", bodyLimit("10kb"), queryLimiter, validateBody(weightSchema), (async (req, res) => {
    const body = req.body;
    const snapshotTime = body.snapshotTime ?? Math.floor(Date.now() / 1000);
    const ageDays = body.ageDays ?? ageDaysAt(body.mintedAt, snapshotTime);
 ...
 */
router.post("/sybil/simulate", bodyLimit("10kb"), queryLimiter, validateBody(simulateSchema), (async (req, res) => {
