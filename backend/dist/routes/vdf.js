...
import { computeVoteCommitment, createVoteCommitment, verifyVoteCommitment, delayProfiles, commitRevealCostAnalysis, benchmarkVdf, MIN_BLINDING_BYTES, MIN_VDF_ITERATIONS, MAX_VDF_ITERATIONS, MAX_ON_CHAIN_HASHES, SOROBAN_CPU_BUDGET, SHA256_COST_INSTRUCTIONS, ASSUMED_ATTACKER_SPEEDUP, } from "../services/vdf.js";
const router = Router();
 ...
 */
router.get("/vdf/profiles", queryLimiter, (async (_req, res) => {
    return res.json({
 ...
 */
router.get("/vdf/cost-analysis", queryLimiter, (async (_req, res) => {
    return res.json({
 ...
 */
router.post("/vdf/commitment", bodyLimit("10kb"), voteLimiter, validateBody(commitmentSchema), (async (req, res) => {
    const { daoId, proposalId, choice, blinding } = req.body;
    if (blinding) {
        const commitment = computeVoteCommitment(daoId, proposalId, choice, blinding);
    ...
    return res.json({
        ...
        warning: "Store this blinding factor. The vote cannot be revealed without it, " +
 ...
 */
router.post("/vdf/commitment/verify", bodyLimit("10kb"), queryLimiter, validateBody(verifySchema), (async (req, res) => {
    const { commitment, daoId, proposalId, choice, blinding } = req.body;
    const valid = verifyVoteCommitment(commitment, daoId, proposalId, choice, blinding);
 ...
 */
router.post("/vdf/benchmark", bodyLimit("10kb"), queryLimiter, validateBody(benchmarkSchema), (async (req, res) => {
