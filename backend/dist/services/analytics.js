 ...
 */
export function partitionExists(daoId) {
    const row = getDb()
        .prepare("SELECT 1 AS ok FROM sqlite_master WHERE type = 'table' AND name = ?")
        .get(partitionTable(daoId));
 ...
 */
export function ensureAnalyticsIndexes(daoId) {
...
function emptyOverview(daoId, memberCount) {
    return {
        daoId,
        memberCount,
        proposalsCreated: 0,
 ...
 */
export async function getDaoOverview(daoId) {
    const table = partitionTable(daoId);
    if (!partitionExists(daoId)) {
  ...
  `.execute(kysely);
    ...
    return {
        ...
        averageVotesPerProposal: proposalsCreated > 0 ? votesCast / proposalsCreated : null,
        proposalParticipationRate: proposalsCreated > 0 ? proposalsWithVotes / proposalsCreated : null,
    };
}
/** Cached member count for the DAO, used as the turnout denominator. */
async function getMemberCount(daoId) {
 ...
 */
export async function getProposalTurnout(daoId, query = {}) {
    const table = partitionTable(daoId);
    ...
    const proposalFilter = query.proposalId === undefined
        ? sql.raw("")
  ...
  `.execute(kysely);
    ...
    const totalFilter = query.proposalId === undefined
        ? sql.raw("")
  ...
  `.execute(kysely);
    return {
        items: result.rows.map((row) => ({
            ...
            eligibleVoters,
            turnoutRatio: row.turnout_ratio === null ? null : Number(row.turnout_ratio),
 ...
 */
export async function getParticipationTimeseries(daoId, query = {}) {
    const table = partitionTable(daoId);
    if (!partitionExists(daoId))
        return [];
    ensureAnalyticsIndexes(daoId);
...
export async function getPlatformOverview() {
    ...
    const partitionSelects = daoIds.map((daoId) => sql `SELECT type, timestamp FROM ${sql.table(partitionTable(daoId))} WHERE verified = 1`);
    const result = await sql `
    SELECT
      SUM(CASE WHEN type = 'proposal_created' THEN 1 ELSE 0 END) AS proposals_created,
      SUM(CASE WHEN type = 'vote_cast' THEN 1 ELSE 0 END) AS votes_cast,
...
export function turnoutToCsv(rows) {
 ...
 */
export function logAnalyticsQuery(metric, daoId, durationMs, rowCount) {
