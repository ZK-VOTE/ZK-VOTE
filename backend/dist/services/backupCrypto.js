...
export function readHeader(filePath) {
    ...
    try {
        ...
        throw new BackupCryptoError("IO_ERROR", `Unable to open backup for reading: ${err.message}`);
    }
    try {
        const preamble = Buffer.alloc(PREAMBLE_LENGTH);
        const read = fs.readSync(descriptor, preamble, 0, PREAMBLE_LENGTH, 0);
        if (read < PREAMBLE_LENGTH) {
            throw new BackupCryptoError("MALFORMED_HEADER", "Backup is shorter than the container preamble");
        }
        const magic = preamble.toString("ascii", 0, 4);
        if (magic !== BACKUP_MAGIC) {
            throw new BackupCryptoError("INVALID_MAGIC", `Not an encrypted ZK-VOTE backup (magic "${magic}")`);
        }
        const version = preamble.readUInt8(4);
        if (version !== BACKUP_FORMAT_VERSION) {
            throw new BackupCryptoError("UNSUPPORTED_VERSION", `Unsupported encrypted backup version ${version}`);
        }
        const headerLength = preamble.readUInt32BE(7);
        if (headerLength <= 0 || headerLength > 1024 * 1024) {
            throw new BackupCryptoError("MALFORMED_HEADER", `Invalid header length ${headerLength}`);
        }
        const headerBuf = Buffer.alloc(headerLength);
        const headerRead = fs.readSync(descriptor, headerBuf, 0, headerLength, PREAMBLE_LENGTH);
        if (headerRead < headerLength) {
            throw new BackupCryptoError("MALFORMED_HEADER", "Truncated backup header");
        }
        let header;
        try {
            ...
            throw new BackupCryptoError("MALFORMED_HEADER", `Backup header is not valid JSON: ${err.message}`);
        }
        if (!header.keyId ||
            !header.kdf?.saltB64 ||
            !header.dek?.ivB64 ||
            ...
            throw new BackupCryptoError("MALFORMED_HEADER", "Backup header is missing required fields");
        }
        return header;
    }
    finally {
 ...
 */
export function probeBackupFile(filePath) {
    if (!fs.existsSync(filePath)) {
        throw new BackupCryptoError("IO_ERROR", `Backup does not exist: ${filePath}`);
    }
    let descriptor;
    try {
        descriptor = fs.openSync(filePath, "r");
        ...
        throw new BackupCryptoError("IO_ERROR", `Unable to open backup for probing: ${err.message}`);
    }
    try {
...
export async function encryptBackupFile(inputPath, outputPath, key) {
    if (!fs.existsSync(inputPath)) {
        throw new BackupCryptoError("IO_ERROR", `Input file does not exist: ${inputPath}`);
    }
    ...
    try {
        ...
        throw new BackupCryptoError("IO_ERROR", `Encryption failed: ${err.message}`);
    }
}
/**
 ...
 */
export async function decryptBackupFile(inputPath, outputPath, key, keyId) {
    const header = readHeader(inputPath);
    if (keyId !== undefined && keyId !== header.keyId) {
        throw new BackupCryptoError("WRONG_KEY", `Key ${deriveKeyId(key)} does not match backup key ${header.keyId}`);
    }
    ...
    try {
        ...
        throw new BackupCryptoError("WRONG_KEY", "Failed to unwrap the backup data key (wrong key or corrupted header)");
    }
    // Locate the GCM auth tag at the very end of the payload.
    const payloadOffset = PREAMBLE_LENGTH + Buffer.byteLength(JSON.stringify(header), "utf-8");
    const fileSize = fs.statSync(inputPath).size;
    if (payloadOffset + header.payload.size + TAG_LENGTH !== fileSize) {
        throw new BackupCryptoError("CORRUPT_PAYLOAD", `Backup size mismatch (expected ${payloadOffset + header.payload.size + TAG_LENGTH}, got ${fileSize})`);
    }
    let descriptor;
    try {
        ...
        throw new BackupCryptoError("IO_ERROR", `Unable to open backup for decryption: ${err.message}`);
    }
    try {
        const tag = Buffer.alloc(TAG_LENGTH);
        fs.readSync(descriptor, tag, 0, TAG_LENGTH, payloadOffset + header.payload.size);
        ...
        try {
            ...
            throw new BackupCryptoError("WRONG_KEY", "Authentication failed while decrypting backup (wrong key or corrupt payload)");
        }
    }
    finally {
        fs.closeSync(descriptor);
 ...
 */
export function logBackupCryptoEvent(level, event, meta = {}) {
