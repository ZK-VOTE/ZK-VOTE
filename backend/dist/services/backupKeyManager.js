...
export function currentKeyFilePath() {
    return config.backupEncryptionKeyFile || defaultKeyFile();
}
export function keyRingDir() {
...
function readKeyLines(filePath) {
    if (!filePath || !fs.existsSync(filePath))
        return [];
    const content = fs.readFileSync(filePath, "utf-8");
    return parseKeyFile(content);
}
function ensureSecureKeyFile(filePath, content) {
...
export function getCurrentBackupKey() {
...
export function getCandidateBackupKeys() {
    const candidates = new Map();
    const current = getCurrentBackupKey();
    if (current)
        candidates.set(current.keyId, current);
 ...
 */
export function ensureBackupEncryptionKey() {
    const existing = getCurrentBackupKey();
    if (existing)
        return existing;
    if (!config.backupEncryptionAutoInit)
        return null;
    const newKey = generateBackupKey();
    const keyFile = currentKeyFilePath();
    ensureSecureKeyFile(keyFile, `# ZKVOTE backup encryption key (v1)\n${newKey}\n`);
    const entry = {
    ...
    log("warn", "backup_encryption_key_autogenerated", {
        keyId: entry.keyId,
        keyFile,
        reason: "BACKUP_ENCRYPTION_AUTO_INIT=true and no key configured. Copy this key into " +
...
// STATE / AUDIT
// ============================================
export function getBackupEncryptionState() {
    const current = getCurrentBackupKey();
    ...
    if (fs.existsSync(ringDir)) {
        try {
            archivedKeys = fs
                ...
                .filter((e) => e.isFile() && e.name.endsWith(".key") && e.name !== currentFileName).length;
        }
        catch {
            /* ignore */
        }
...
function recordKeyEvent(params) {
    ...
    try {
        if (!isDbInitialized())
            return;
        const db = getDb();
        db.prepare(`INSERT OR REPLACE INTO backup_keys
         (key_id, created_at, source, current, rotated_from, event_type)
       VALUES (?, ?, ?, ?, ?, ?)`).run(keyId, new Date().toISOString(), source, "current", rotatedFrom ?? null, action);
    }
    catch {
        // The table may not exist yet (migration pending) — metadata is optional.
    }
}
export function logKeyEvent(level, event, meta) {
 ...
 */
export function rotateBackupEncryptionKey(options = {}) {
    const current = getCurrentBackupKey();
    if (!current) {
        throw new BackupKeyError("No backup encryption key is configured. Generate one first " +
            "(backup-key-manager.ts generate) or set BACKUP_ENCRYPTION_KEY.");
    }
    if (current.source === "env") {
        log("warn", "backup_key_rotation_requires_output_file", {
            reason: "Current key comes from BACKUP_ENCRYPTION_KEY (env). The new key is " +
    ...
    if (!fs.existsSync(archivedPath)) {
        ensureSecureKeyFile(archivedPath, `# ZKVOTE archived backup encryption key (${current.keyId})\n${current.key}\n`);
    }
    recordKeyEvent({
        keyId: current.keyId,
        action: "archived",
    ...
    ensureSecureKeyFile(outputFile, `# ZKVOTE backup encryption key (v1)\n${newKey}\n`);
    recordKeyEvent({
        keyId: newKeyId,
        action: "rotated-in",
        source: outputFile,
...
//# sourceMappingURL=backupKeyManager.js.map