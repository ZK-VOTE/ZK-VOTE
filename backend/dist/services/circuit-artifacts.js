...
export const GENERATED_DEPTHS = [10, 15, 20, 25];
/** Every depth the protocol accepts, including the default circuit. */
export const SUPPORTED_DEPTHS = [
    ...GENERATED_DEPTHS,
 ...
 */
export function circuitsBuildDir() {
 ...
 */
export function resolveDepth(merkleDepth) {
    ...
    if (merkleDepth > MAX_MERKLE_DEPTH) {
        throw new Error(`merkle_depth ${merkleDepth} exceeds the maximum of ${MAX_MERKLE_DEPTH}`);
    }
    return merkleDepth;
}
/**
 ...
 */
export function resolveArtifacts(merkleDepth) {
    ...
    if (!SUPPORTED_DEPTHS.includes(depth)) {
        throw new Error(`no circuit compiled for Merkle depth ${depth} ` +
...
export function missingArtifacts(artifacts) {
    return [artifacts.wasmPath, artifacts.zkeyPath, artifacts.vkeyPath].filter((p) => !fs.existsSync(p));
}
const depthCache = new Map();
const DEPTH_TTL_MS = 60_000;
export function invalidateDepthCache(daoId, proposalId) {
    if (daoId === undefined || proposalId === undefined)
        depthCache.clear();
    else
        depthCache.delete(`${daoId}:${proposalId}`);
}
/**
 ...
 */
export async function getProposalMerkleDepth(daoId, proposalId) {
    ...
    try {
        ...
        const tx = new StellarSdk.TransactionBuilder(account, {
            ...
            .addOperation(contract.call("get_merkle_depth", StellarSdk.nativeToScVal(daoId, { type: "u64" }), StellarSdk.nativeToScVal(proposalId, { type: "u64" })))
            .setTimeout(30)
            .build();
        const result = await callWithTimeout(() => rpcServer.simulateTransaction(tx), "get_merkle_depth");
 ...
 */
export async function resolveArtifactsForProposal(daoId, proposalId) {
    const depth = await getProposalMerkleDepth(daoId, proposalId);
    return resolveArtifacts(depth ?? 0);
}
//# sourceMappingURL=circuit-artifacts.js.map