 ...
 */
export function resolveDualWriteMode() {
    if (!config.dbDualWrite)
        return "off";
 ...
 */
export function initDualWrite(executor, mode) {
    shadow = executor;
    if (mode) {
...
export function resetDualWriteStats() {
    stats.mirrored = 0;
    stats.failed = 0;
    stats.skipped = 0;
    stats.backfilled = 0;
 ...
 */
export async function mirrorWrite(sql, params = []) {
 ...
 */
export async function mirrorBatch(statements) {
    if (!isDualWriteEnabled()) {
 ...
 */
export async function verify(tables, countPrimary) {
    if (!isDualWriteEnabled())
        return [];
    const divergences = [];
    for (const table of tables) {
        const primaryCount = await countPrimary(table);
        const { rows } = await shadow.query(`SELECT COUNT(*)::bigint AS count FROM ${quoteIdent(table)}`);
 ...
 */
export async function backfill(table, rows, conflictColumns) {
    if (!isDualWriteEnabled() || rows.length === 0)
        return 0;
    let inserted = 0;
    for (const row of rows) {
        ...
        try {
            await shadow.query(sql, columns.map((c) => row[c]));
            inserted += 1;
        }
        catch (err) {
            stats.failed += 1;
