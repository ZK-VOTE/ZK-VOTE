...
export const CONTENT_TYPES = ["proposal", "comment"];
// ============================================
...
export function combineShares(shares) {
    if (shares.length === 0)
        throw new Error("No shares supplied");
 ...
 */
export function keyCommitment(groupKey) {
 ...
 */
export function deriveMemberKey(memberSecret, daoId, memberId) {
    return Buffer.from(hkdfSync("sha256", memberSecret, Buffer.from(`zkvote/e2e/v1/member-salt/${daoId}`), Buffer.from(`zkvote/e2e/v1/member-key/${daoId}/${memberId}`), KEY_BYTES));
}
function wrapDomain(daoId, epoch, memberId) {
...
export function wrapGroupKeyForMember(groupKey, memberKey, daoId, epoch, memberId) {
    ...
    return {
        ...
        epoch,
        memberId,
        wrapped: Buffer.concat([nonce, ciphertext, cipher.getAuthTag()]).toString("base64"),
    };
}
/**
 ...
 * garbage.
 */
export function unwrapGroupKeyForMember(wrapped, memberKey, daoId, epoch, memberId) {
    assertKeyLength(memberKey, "member key");
...
function assertKeyLength(key, label) {
 ...
 */
export function contentDomain(daoId, epoch, contentType, contentId) {
 ...
 */
export function deriveNonce(domain) {
...
export function encryptContent(groupKey, params) {
    assertKeyLength(groupKey, "group key");
    if (!CONTENT_TYPES.includes(params.contentType)) {
        throw new Error(`Unsupported content type: ${params.contentType}`);
    }
    const domain = contentDomain(params.daoId, params.epoch, params.contentType, params.contentId);
    const nonce = deriveNonce(domain);
 ...
 */
export function decryptContent(groupKey, envelope) {
    assertKeyLength(groupKey, "group key");
    if (envelope.v !== ENVELOPE_VERSION) {
        throw new Error(`Unsupported envelope version: ${String(envelope.v)}`);
    }
    const domain = contentDomain(envelope.daoId, envelope.epoch, envelope.contentType, envelope.contentId);
    const decipher = createDecipheriv(CIPHER, groupKey, Buffer.from(envelope.nonce, "base64"));
 ...
 */
export function redactEnvelopeForLog(envelope) {
    return {
        v: envelope.v,
...
export function initEncryptionSchema() {
    getDb().exec(`
...
export function recordGroupKeyEpoch(params) {
    ...
    const insert = db.transaction(() => {
        db.prepare("UPDATE dao_group_keys SET active = 0 WHERE dao_id = ?").run(params.daoId);
        db.prepare(`INSERT INTO dao_group_keys
         (dao_id, epoch, threshold, member_count, key_commitment, rotation_reason, active)
       VALUES (?, ?, ?, ?, ?, ?, 1)`).run(params.daoId, params.epoch, params.threshold, params.wraps.length, params.keyCommitment, params.rotationReason);
        const wrapStatement = db.prepare(`INSERT OR REPLACE INTO dao_key_wraps (dao_id, epoch, member_id, wrapped_key)
       VALUES (?, ?, ?, ?)`);
        for (const wrap of params.wraps) {
            wrapStatement.run(params.daoId, params.epoch, wrap.memberId, wrap.wrapped);
        }
        const shareStatement = db.prepare(`INSERT OR REPLACE INTO dao_recovery_shares (dao_id, epoch, share_index, wrapped_share)
       VALUES (?, ?, ?, ?)`);
        for (const share of params.recoveryShares) {
            shareStatement.run(params.daoId, params.epoch, share.index, share.wrappedShare);
        }
    });
    insert();
...
export function getActiveEpoch(daoId) {
    initEncryptionSchema();
    const row = getDb()
        .prepare(`SELECT dao_id, epoch, threshold, member_count, key_commitment,
...
export function nextEpoch(daoId) {
    initEncryptionSchema();
    const row = getDb()
 ...
 */
export function getWrappedKey(daoId, epoch, memberId) {
    initEncryptionSchema();
    const row = getDb()
        .prepare(`SELECT wrapped_key FROM dao_key_wraps
        WHERE dao_id = ? AND epoch = ? AND member_id = ?`)
        .get(daoId, epoch, memberId);
...
export function getRecoveryShares(daoId, epoch) {
    initEncryptionSchema();
    const rows = getDb()
        .prepare(`SELECT share_index, wrapped_share FROM dao_recovery_shares
...
export function storeCiphertext(envelope) {
    initEncryptionSchema();
    getDb()
        .prepare(`INSERT INTO encrypted_content
       ...
       ON CONFLICT(dao_id, content_type, content_id) DO UPDATE SET
        ...
        .run(envelope.daoId, envelope.contentType, envelope.contentId, envelope.epoch, envelope.nonce, envelope.ciphertext, envelope.tag);
...
export function loadCiphertext(daoId, contentType, contentId) {
    initEncryptionSchema();
    const row = getDb()
        .prepare(`SELECT dao_id, content_type, content_id, epoch, nonce, ciphertext, tag,
              redacted, redacted_at, redaction_reason, created_at
         FROM encrypted_content
        WHERE dao_id = ? AND content_type = ? AND content_id = ?`)
        .get(daoId, contentType, contentId);
    if (!row)
        return null;
    ...
    return {
        ...
        envelope: redacted || row.nonce === null || row.ciphertext === null || row.tag === null
            ? null
            : {
                v: ENVELOPE_VERSION,
                daoId: row.dao_id,
 ...
 */
export function redactContent(daoId, contentType, contentId, reason) {
    initEncryptionSchema();
    const result = getDb()
        .prepare(`UPDATE encrypted_content
          SET nonce = NULL,
              ciphertext = NULL,
              tag = NULL,
              redacted = 1,
 ...
 */
export function verifyGroupKey(groupKey, epoch) {
