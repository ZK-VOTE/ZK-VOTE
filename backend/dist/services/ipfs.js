...
import { ipfsPinsTotal, ipfsFetchDuration, ipfsCacheHits, ipfsCacheMisses, } from "./metrics.js";
...
const STANDALONE_TAG_RE = /<\/?\s*(?:script|style|svg|math|iframe|object|embed|applet|base|link|meta|form|input|button|select|textarea|option|frame|frameset|video|audio|source|track|param|isindex|keygen|marquee|animate|animatemotion|animatetransform|set|use|foreignobject|discard|listener|handler)\b[^>]*>?/gi;
...
const JS_SCHEME_RE = /j[\s]*a[\s]*v[\s]*a[\s]*s[\s]*c[\s]*r[\s]*i[\s]*p[\s]*t[\s]*:/gi;
const VB_SCHEME_RE = /v[\s]*b[\s]*s[\s]*c[\s]*r[\s]*i[\s]*p[\s]*t[\s]*:/gi;
const LIVESCRIPT_SCHEME_RE = /l[\s]*i[\s]*v[\s]*e[\s]*s[\s]*c[\s]*r[\s]*i[\s]*p[\s]*t[\s]*:/gi;
...
const SCRIPTABLE_DATA_URL_RE = /data[\s]*:[\s]*(?:text[\s]*\/[\s]*html|image[\s]*\/[\s]*svg[\s]*\+[\s]*xml|application[\s]*\/[\s]*(?:xhtml[\s]*\+[\s]*xml|x-shockwave-flash))/gi;
...
function decodeHtmlEntities(input) {
    ...
    for (let i = 0; i < MAX_ENTITY_DECODE_ROUNDS; i++) {
        const next = out
            .replace(/&#[xX]([0-9a-fA-F]{1,6});?/g, (_m, hex) => codePointToString(parseInt(hex, 16)))
            .replace(/&#(\d{1,7});?/g, (_m, dec) => codePointToString(parseInt(dec, 10)))
 ...
 */
function sanitizeEmbeddedJson(value, jsonDepth) {
    if (jsonDepth >= MAX_EMBEDDED_JSON_DEPTH)
        return value;
 ...
 */
function sanitizeValue(value, objectDepth, jsonDepth) {
    ...
    if (Array.isArray(value)) {
        return value.map((entry) => sanitizeValue(entry, objectDepth + 1, jsonDepth));
    }
    const sanitized = {};
    for (const [key, entryValue] of Object.entries(value)) {
        ...
        sanitized[sanitizedKey] = sanitizeValue(entryValue, objectDepth + 1, jsonDepth);
    }
    return sanitized;
}
/**
...
export async function pinJSON(data, name = "zkvote-metadata") {
    ...
    try {
        ...
        log("warn", "local_backup_failed", { name, error: err.message });
        // Non-fatal — continue with the pin
    }
    // 2. Upload to Pinata (primary)
    // SDK v2.x: pinata.upload.public.json() with chainable methods
    const result = await pinataBreaker.execute(async () => pinata.upload.public.json(data).name(name).keyvalues({
        app: "zkvote",
    ...
    try {
        pinManager.registerPin(result.cid, "json", name, sizeBytes, "application/json", backupPath);
    }
    catch (err) {
...
export async function pinFile(buffer, filename, mimeType) {
    ...
    const result = await pinataBreaker.execute(async () => pinata.upload.public.file(file).name(filename).keyvalues({
        app: "zkvote",
        type: "proposal-image",
    }));
 ...
 */
export function sanitizeCid(cid) {
 ...
 */
export function isAllowedGatewayUrl(urlString) {
    try {
 ...
 */
async function fetchFromPublicGateways(cleanCid) {
    let lastError;
    for (const gateway of PUBLIC_GATEWAYS) {
        ...
        try {
            ...
            if (!res.ok) {
                throw new Error(`Failed to fetch from IPFS: ${res.status} ${res.statusText}`);
            }
            return res;
        }
        catch (err) {
...
export async function fetchContent(cid) {
    ...
    try {
        ...
        try {
            ...
            const contentType = fallbackRes.headers.get("content-type") || "application/json";
            if (contentType.includes("text/html") ||
                contentType.includes("application/javascript") ||
                contentType.includes("text/javascript")) {
                throw new Error(`Forbidden response content-type: ${contentType}`, {
 ...
 */
export function getEnhancedHealth() {
 ...
 */
export async function repinCallback(backupPath, contentType, name, mimeType) {
    if (!pinata) {
        throw new Error("Pinata client not initialized");
    }
    if (contentType === "json") {
