...
function shouldSkipNoopAddColumn(database, sql) {
    const match = sql.match(/ALTER\s+TABLE\s+([A-Za-z_][A-Za-z0-9_]*)\s+ADD\s+COLUMN\s+([A-Za-z_][A-Za-z0-9_]*)/i);
    if (!match) {
        return { skip: false };
    }
 ...
 */
async function runPostgresCli(command, args) {
    const { config } = await import("../config.js");
    const { createPgPool } = await import("./dbDialect.js");
    const { migrateUpPg, migrateDownPg, getAppliedMigrationsPg } = await import("./migratePg.js");
...
async function cli() {
    ...
    if (command === "parity") {
        ...
        for (const e of report.entries) {
            ...
            console.info(`  ${mark} ${e.id.padEnd(5)} sqlite=${String(e.sqliteName).padEnd(32)} postgres=${e.postgresName}`);
        }
        console.info("-".repeat(70));
        if (report.inParity) {
