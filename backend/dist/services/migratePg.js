...
import { sqlFlavorFor } from "./dbDialect.js";
 ...
 */
export function migrationsDirFor(flavor = sqlFlavorFor()) {
    return flavor === "postgres" ? POSTGRES_MIGRATIONS_DIR : SQLITE_MIGRATIONS_DIR;
}
// ============================================
// STATE TRACKING
// ============================================
async function ensureMigrationsTable(exec) {
    await exec.query(`
...
export async function getAppliedMigrationsPg(exec) {
    await ensureMigrationsTable(exec);
    const { rows } = await exec.query("SELECT id, applied_at, checksum, duration_ms FROM _migrations ORDER BY id ASC");
    return rows;
}
// ============================================
// LOCK
...
// EXECUTION
// ============================================
async function applyMigration(exec, migration, direction) {
    ...
    try {
        ...
        try {
            ...
            if (direction === "up") {
                await exec.query(`INSERT INTO _migrations (id, applied_at, checksum, duration_ms)
...
function dryRun(migration, direction) {
    const sql = direction === "up" ? migration.up : migration.down;
...
// MIGRATE UP / DOWN
// ============================================
export async function migrateUpPg(exec, options = {}) {
    const results = [];
    const migrations = loadMigrations(options.migrationsDir ?? POSTGRES_MIGRATIONS_DIR);
    ...
    if (!options.dryRun && !(await acquireLock(exec))) {
        throw new Error("Could not acquire the Postgres migration advisory lock — another " +
...
export async function migrateDownPg(exec, options = {}) {
    const results = [];
    const migrations = loadMigrations(options.migrationsDir ?? POSTGRES_MIGRATIONS_DIR);
    const applied = await getAppliedMigrationsPg(exec);
 ...
 */
export function checkMigrationParity(sqliteDir = SQLITE_MIGRATIONS_DIR, postgresDir = POSTGRES_MIGRATIONS_DIR) {
    ...
    for (const id of ids) {
        ...
        if (s && p && s.name !== p.name)
            nameMismatches.push(id);
        if ((s && !s.down.trim()) || (p && !p.down.trim()))
            missingDown.push(id);
        entries.push({
            id,
    ...
    return {
        ...
        nameMismatches,
        missingDown,
        inParity: missingInPostgres.length === 0 &&
            missingInSqlite.length === 0 &&
            nameMismatches.length === 0 &&
            missingDown.length === 0,
    };
}
//# sourceMappingURL=migratePg.js.map