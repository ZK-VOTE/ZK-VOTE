 ...
 */
export function initializeAnalytics(cfg) {
    ...
    const tx = writeDb.transaction(() => {
        ...
        if (existing) {
            if (existing.joint_public_key !== cfg.jointPublicKey) {
                throw new PrivacyAnalyticsError("Joint public key mismatch for existing analytics aggregate");
            }
            return existing;
        }
        writeDb
            .prepare(`INSERT OR REPLACE INTO analytics_aggregates
         ...
         VALUES (?, ?, ?, ?, 0, ?, ?, 0, NULL, NULL, ?)`)
            .run(cfg.daoId, cfg.jointPublicKey, "0", // c1 = point at infinity (identity)
        "0", cfg.thresholdT, cfg.thresholdN, isoNow());
        // Seed the privacy budget row.
        writeDb
            .prepare(`INSERT OR REPLACE INTO privacy_budget
          (dao_id, epsilon_budget, epsilon_spent, epsilon_per_query,
           min_cohort, window_started_at, updated_at)
         VALUES (?, ?, 0, ?, ?, ?, ?)`)
            .run(cfg.daoId, cfg.epsilonBudget ?? DEFAULT_EPSILON_BUDGET, cfg.epsilonPerQuery ?? DEFAULT_EPSILON_PER_QUERY, minCohort, isoNow(), isoNow());
    });
    tx();
 ...
 */
export function accumulateContribution(daoId, contribution) {
    validateDaoId(daoId);
    const writeDb = getWriteDb();
    ...
    const next = row.contribution_count === 0
        ? contribution
        : tc.homomorphicAdd({ c1: row.aggregate_c1, c2: row.aggregate_c2 }, contribution);
    writeDb
        .prepare(`UPDATE analytics_aggregates
 ...
 */
export function getState(daoId) {
    validateDaoId(daoId);
    const row = getAggregateRow(daoId);
    if (!row) {
        throw new PrivacyAnalyticsError("Analytics not initialized for DAO");
 ...
 */
export function getPrivacyBudget(daoId) {
    validateDaoId(daoId);
 ...
 */
export function thresholdDecryptAggregate(daoId, shares) {
    validateDaoId(daoId);
    const writeDb = getWriteDb();
    ...
    if (uniqueShares.length < agg.threshold_t) {
        throw new PrivacyAnalyticsError(`Insufficient decryption shares: have ${uniqueShares.length}, need ${agg.threshold_t}`);
    }
    ...
    if (agg.contribution_count < aggGuard(daoId).min_cohort) {
        throw new PrivacyAnalyticsError(`Cohort too small to decrypt: ${agg.contribution_count} < ${aggGuard(daoId).min_cohort}`);
    }
 ...
 */
export function resetPrivacyBudget(daoId, newBudget) {
    validateDaoId(daoId);
    const writeDb = getWriteDb();
    writeDb
        .prepare(`UPDATE privacy_budget
       ...
       WHERE dao_id = ?`)
        .run(newBudget ?? DEFAULT_EPSILON_BUDGET, isoNow(), isoNow(), daoId);
    return getPrivacyBudget(daoId);
}
// ── Internal helpers ──────────────────────────────────────────────────
function getAggregateRow(daoId) {
    return getReadDb()
        .prepare("SELECT * FROM analytics_aggregates WHERE dao_id = ?")
        .get(daoId);
}
function aggGuard(daoId) {
    const readDb = getReadDb();
    return (readDb
        .prepare("SELECT * FROM privacy_budget WHERE dao_id = ?")
        .get(daoId) ?? {
...
function aggEpsilonPerQuery(daoId) {
...
//# sourceMappingURL=privacy-analytics.js.map