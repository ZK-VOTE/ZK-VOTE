...
export function createSignedSessionToken(clientId, privateKeyPem, options = {}) {
...
export function verifySignedSessionToken(sessionToken, expectedDaoId) {
    ...
    try {
        ...
        const ok = crypto.verify(null, Buffer.from(payloadJson), publicKey, signature);
        if (!ok) {
            return {
                valid: false,
...
export function getDefaultSessionSigningKey() {
    const key = config.relayerSecretKey ||
...
//# sourceMappingURL=relaySessions.js.map