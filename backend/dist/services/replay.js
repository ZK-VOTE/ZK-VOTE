...
export function digestState(value) {
 ...
 */
export function redactInteractionPayload(payload) {
    const out = {};
    for (const [key, value] of Object.entries(payload)) {
        ...
        if (value !== null && typeof value === "object" && !Array.isArray(value)) {
            out[key] = redactInteractionPayload(value);
            continue;
        }
        if (Array.isArray(value)) {
            out[key] = value.map((entry) => entry !== null && typeof entry === "object"
                ? redactInteractionPayload(entry)
                : entry);
            continue;
        }
        out[key] = value;
...
function normaliseState(state) {
    return {
        ...state,
        events: [...state.events].sort((a, b) => eventKey(a) < eventKey(b) ? -1 : eventKey(a) > eventKey(b) ? 1 : 0),
    };
}
// ============================================
// RECORDER
 ...
 * to be re-entrant.
 */
export function startRecording(pipeline, traceId) {
    const resolvedTraceId = traceId ?? getActiveSpanContext()?.traceId ?? "0".repeat(32);
 ...
 */
export function recordInteraction(kind, label, payload) {
    const recorder = activeRecorder;
    if (!recorder)
        return;
...
export function isReplayCaptureEnabled() {
...
export function replayFixture(fixture) {
    ...
    for (const interaction of [...fixture.interactions].sort((a, b) => a.seq - b.seq)) {
...
// FIXTURE I/O
// ============================================
export function writeFixture(filePath, fixture) {
...
export function loadFixture(filePath) {
    ...
    if (parsed.version !== REPLAY_FIXTURE_VERSION) {
        throw new Error(`Unsupported replay fixture version ${String(parsed.version)} (expected ${REPLAY_FIXTURE_VERSION})`);
    }
    return parsed;
}
