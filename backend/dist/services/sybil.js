...
export const AGE_THRESHOLD_DAYS = [7, 30, 90, 180, 365];
/** Reputation thresholds; crossing each adds one point. */
export const REPUTATION_THRESHOLDS = [1, 5, 15, 40, 100];
...
export function agePoints(ageDays) {
    return pointsFor(ageDays, AGE_THRESHOLD_DAYS);
}
/** Points contributed by reputation. */
export function reputationPoints(reputation) {
 ...
 */
export function ageDaysAt(mintedAtSec, snapshotSec) {
    if (snapshotSec <= mintedAtSec)
        return 0;
    return Math.floor((snapshotSec - mintedAtSec) / SECONDS_PER_DAY);
}
/** The full weight computation, with the breakdown the UI needs. */
export function computeWeight(ageDays, reputation) {
 ...
 */
export function analyzeSybilBound(scenario) {
    const weightPerSybil = computeWeight(scenario.sybilAgeDays, scenario.sybilReputation).weight;
    const weightPerHonest = computeWeight(scenario.honestAgeDays, scenario.honestReputation).weight;
    ...
    const attackerShareUnweighted = scenario.sybilCount + scenario.honestCount === 0
        ? 0
    ...
    const sybilsForMajority = Math.ceil((totalHonestWeight + 1) / Math.max(1, weightPerSybil));
    return {
        scenario,
        weightPerSybil,
        weightPerHonest,
        ...
        reductionFactor: attackerShare === 0 ? 0 : attackerShareUnweighted / attackerShare,
    };
}
/** The curve as a table, for docs and for the UI's explainer. */
export function weightCurveTable() {
