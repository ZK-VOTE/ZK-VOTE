...
export function createSpanContext(parent) {
    return {
...
function isSensitiveKey(key) {
    ...
    return SENSITIVE_ATTRIBUTE_PATTERNS.some((pattern) => normalized.includes(pattern.replace(/[^a-z0-9]/g, "")));
}
/**
 ...
 */
export function redactSpanAttributes(attributes) {
    const redacted = {};
 ...
 */
export function registerSpanExporter(exporter) {
...
export function clearSpanExporters() {
    exporters.clear();
}
 ...
 */
export function runWithSpanContext(context, fn) {
 ...
 */
export async function withSpan(name, attributes, operation, options = {}) {
    const parent = options.parent === undefined ? getActiveSpanContext() : options.parent;
