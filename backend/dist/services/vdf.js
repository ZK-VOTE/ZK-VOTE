 ...
 */
export function computeVoteCommitment(daoId, proposalId, nullifier, choice, blindingHex) {
    const blinding = Buffer.from(blindingHex, "hex");
    if (blinding.length < MIN_BLINDING_BYTES) {
        throw new Error(`Vote commitment blinding must be at least ${MIN_BLINDING_BYTES} bytes; ` +
    ...
    const nullifierBuf = Buffer.from(BigInt(nullifier).toString(16).padStart(64, "0"), "hex");
    if (nullifierBuf.length !== 32) {
        throw new Error(`Nullifier must encode to exactly 32 bytes; got ${nullifierBuf.length}.`);
    }
...
export function createVoteCommitment(daoId, proposalId, nullifier, choice) {
    const blinding = crypto.randomBytes(MIN_BLINDING_BYTES).toString("hex");
    return {
        commitment: computeVoteCommitment(daoId, proposalId, nullifier, choice, blinding),
        blinding,
        daoId,
        proposalId,
        nullifier,
 ...
 * commitment matched.
 */
export function verifyVoteCommitment(commitmentHex, daoId, proposalId, nullifier, choice, blindingHex) {
    let expected;
    try {
        expected = computeVoteCommitment(daoId, proposalId, nullifier, choice, blindingHex);
    }
    catch {
        return false;
    }
 ...
 */
export function delayProfiles(hashesPerSec = REFERENCE_HASHES_PER_SEC) {
    const profiles = [
        {
            name: "minimal",
            iterations: 1_000_000,
            checkpoints: 8,
            suitableFor: "Testing and low-stakes polls. Too short to deter a determined coercer.",
        },
        {
            name: "standard",
            iterations: 600_000_000,
            checkpoints: 32,
            suitableFor: "Ordinary DAO proposals. ~1 minute for the reference prover, ~6s for a 10x adversary — enough that a coercer cannot verify compliance inside a typical voting window.",
        },
        {
            name: "high",
            iterations: 6_000_000_000,
            checkpoints: 64,
            suitableFor: "Treasury and constitutional votes. ~10 minutes reference, ~1 minute adversarial.",
        },
        {
            name: "maximum",
            iterations: 36_000_000_000,
            checkpoints: 100,
            suitableFor: "Elections where coercion is the primary threat. ~1 hour reference. Reveals must be relayed, since no voter will keep a tab open.",
        },
    ];
...
export const MAX_ON_CHAIN_HASHES = Math.floor(SOROBAN_CPU_BUDGET / SHA256_COST_INSTRUCTIONS);
/**
 ...
 */
export function commitRevealCostAnalysis(hashesPerSec = REFERENCE_HASHES_PER_SEC) {
    return delayProfiles(hashesPerSec).map((p) => {
        ...
        return {
            ...
            notes: spotChecked <= MAX_ON_CHAIN_HASHES
