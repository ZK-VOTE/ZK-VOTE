...
async function hasMalwareIndicators(buffer) {
 ...
 */
export const bn254Field = z.string().refine((val) => {
    const hex = val.startsWith("0x") ? val.slice(2) : val;
 ...
 */
const bn254FieldAnon = z.string().refine((val) => {
    const hex = val.startsWith("0x") ? val.slice(2) : val;
    if (hex.length === 0 || hex.length > 64)
        return false;
    if (!/^[0-9a-fA-F]*$/.test(hex))
 ...
 */
const proofAAnon = hexString(128).refine((val) => {
    const hex = val.startsWith("0x") ? val.slice(2) : val;
    const padded = hex.padStart(128, "0");
    if (/^0*$/.test(padded))
        return false;
...
const proofBAnon = hexString(256).refine((val) => {
    const hex = val.startsWith("0x") ? val.slice(2) : val;
    const padded = hex.padStart(256, "0");
    if (/^0*$/.test(padded))
        return false;
...
const proofCAnon = hexString(128).refine((val) => {
    const hex = val.startsWith("0x") ? val.slice(2) : val;
    const padded = hex.padStart(128, "0");
    if (/^0*$/.test(padded))
        return false;
 ...
 * IPFS CID validator (CIDv0 or CIDv1)
 */
export const ipfsCid = z.string().refine((val) => {
 ...
 */
export const nullifierHex = z.string().refine((val) => {
    const hex = val.startsWith("0x") ? val.slice(2) : val;
    if (hex.length === 0 || hex.length > 64)
        return false;
    return /^[0-9a-fA-F]*$/.test(hex);
 ...
 */
export const commitmentHash = z.string().refine((val) => {
    const hex = val.startsWith("0x") ? val.slice(2) : val;
...
export const blindSignRequestSchema = z.object({
    daoId: z.number().int().nonnegative("daoId must be a non-negative integer"),
    blindedValue: z.string().refine((val) => {
        const hex = val.startsWith("0x") ? val.slice(2) : val;
        if (hex.length === 0 || hex.length > 512)
            return false;
        return /^[0-9a-fA-F]+$/.test(hex);
...
export const voteBatchSchema = z
    ...
    .refine((data) => new Set(data.votes.map((v) => v.nullifier)).size === data.votes.length, {
...
export const qvTallySchema = z.object({
    ballots: z
        .array(z.object({ allocations: z.array(qvAllocationSchema).min(1).max(16) }))
        .min(1),
});
...
export const thresholdDecryptShareSchema = z.object({
    ...
    privateKeyShare: z
        .string()
        .regex(/^-?\d+$|^0[xX][0-9a-fA-F]+$/, "privateKeyShare must be an integer string"),
