 ...
 */
export async function timeQueryAsync<T>(
  operation: string,
  fn: () => Promise<T>,
  extra?: Record<string, unknown>,
): Promise<T> {
 ...
 */
export function getTableStats(database: DatabaseType): DbStats["tables"] {
  try {
    ...
    log("warn", "table_stats_failed", { error: (err as Error).message });
    return [];
  }
}
 ...
 */
export function getDetailedTableStats(
  database: DatabaseType,
): DbStats["tables"] {
  try {
    const tables = database
 ...
 */
export function getCacheStats(): DbStats["cache"] {
  const total = cacheHits + cacheMisses;
  return {
    hits: cacheHits,
    misses: cacheMisses,
 ...
 */
export function getDbStats(database: DatabaseType): DbStats {
 ...
 */
export function profileEventQueries(
  database: DatabaseType,
  daoId: number,
): void {
 ...
 * Get recent alerts.
 */
export function getRecentAlerts(): typeof alertHistory {
  return [...alertHistory];
}
